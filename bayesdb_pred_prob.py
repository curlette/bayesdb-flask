...
def compute_pred_prob(filename, bdb_name, col_names, table="satellites_t", population="satellites_p", columm="Apogee_km", start_row=1):
	...
	create_table_query = "CREATE TABLE user_input FROM %s" % ("'./tmp/%s'" %(filename))
	...
	pred_prob_query = "ESTIMATE PREDICTIVE PROBABILITY OF %s AS pred_prob FROM %s WHERE %s._rowid_ >= %d" %(column, population, table, num_rows + start_row)
	pred_prob_cursor = bdb.execute(pred_prob_query)
	pred_prob_df = cursor_to_df(pred_prob_cursor)
	pred_prob_list = df.loc[:, "pred_prob"].tolist()
